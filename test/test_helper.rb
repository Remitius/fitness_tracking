...
class ActiveSupport::TestCase
  ...
  def valid_workout(name: 'name123', date: 1.day.ago, note: nil, save: true)
    w = Workout.new(name: name, date: date, note: note)
    w.save if save
    w
  end

  def valid_exercise(workout, name: 'name123', note: nil, save: true)
    e = Exercise.new(workout: workout, name: name, note: note)
    e.save if save
    e
  end

  def valid_e_set(exercise, pounds: 500.5, reps: nil, save: true)
